...
export class SideBarApp extends Component {
    ...
    render() {
        return (
            ...
            >
                <img width="28px" height="28px" className="w-7" src={this.props.icon} alt="Fedora App Icon" />
                <img className={(this.state.scaleImage ? " scale " : "") + " scalable-app-icon w-7 absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2"} src={this.props.icon} alt="" />
                {
                    (
                        this.props.isClose[this.id] === false
                            ? <div className=" w-1 h-1 absolute left-0 top-1/2 bg-fd-blue rounded-sm"></div>
                            : null
                    )
                }
                <div
                    className={
                        ...
                        " w-max py-0.5 px-1.5 absolute top-1.5 left-full ml-3 m-1 text-fdt-grey text-opacity-90 text-sm bg-fd-grey bg-opacity-70 border-gray-400 border border-opacity-40 rounded-md"
                    }
                >
                    {this.props.title}
                </div>
